...
# usethis::use_data(hesper_vars, hesper_opts, overwrite = TRUE, internal = TRUE)
...
# # # Prepare datasets
# # hesper_df <- lapply(hesper_vars, \(x) {
# #   x = sample(hesper_opts, 10000, replace = TRUE)
# # }) |>
# #   setNames(hesper_vars) |>
# #   as.data.frame()

# # gender_df <- lapply(gender_vars, \(x) {
# #   x = sample(gender_opts, 10000, replace = TRUE)
# # }) |>
...
# #     # If not empty, sample
# #     if (length(probs)) {
# #       paste(sample(probs, min(3, length(probs))), collapse = " ")
# #     } else {
# #       NA_character_
# #     }
# #   },
...
# #   ) := tstrsplit(hesper_priorities, " ", fixed = TRUE, fill = NA_character_)
# # ]
