...
plot.hist <- function(df, 
  ...
  if (!is.null(fill)) plot <- plot + ggplot2::facet_grid(rows = vars(!!sym(fill)), scales = "free_x")
  if (median) {
    plot <- plot +
...
plot.num <- function(df = median_nb_serious,
  ...
  plot <- df %>%
    dplyr::mutate(!!sym(col_group) := paste0(!!sym(col_group), "\nn=", unique(!!sym(col_n)))) %>%
    ggplot2::ggplot(ggplot2::aes(x=reorder(!!sym(col_group), !!sym(col_stat)), y=!!sym(col_stat))) + 
    ggplot2::geom_point(stat="identity", show.legend = F, color="#EE5859") +
    ggplot2::geom_segment(ggplot2::aes(xend = reorder(!!sym(col_group), -!!sym(col_stat)), yend = 0), color="#EE5859") +
    ggplot2::geom_text(ggplot2::aes(label=round(!!sym(col_stat),1)), hjust=-1, size=3) +
...
create_graph_ggplot2::facet_disag <- function(df=result_hesper, 
  ...
  if (!custom.factor) {df_filtered <- df_filtered %>% dplyr::mutate(choice_label = str_wrap(choice_label,60))}
  ...
  } else {p <- ggplot2::ggplot(df_filtered, ggplot2::aes(x = x_aesthetic, y = mean, fill = !!sym(col_disag))) + ggplot2::geom_bar(position = "dodge", stat = "identity")}
