    ...
    df <- dplyr::mutate(
      ...
      "all_serious_items" := rowSums(dplyr::across(dplyr::any_of(unlist(unname(list_group))), ~ . %in% choice_serious), na.rm=T),
       "n_valid" := rowSums(dplyr::across(dplyr::any_of(list_group[[cat]]), ~ !. %in% choice_exclude)), !!paste0("nb_hesper_items_", list_group_name, ".", cat) := dplyr::case_when(
        ...
        .default = rowSums(dplyr::across(dplyr::any_of(list_group[[cat]]), ~. %in% c(choice_serious)), na.rm=T)
             ),
      !!paste0("prop_hesper_items_", list_group_name, ".", cat) := !!rlang::sym(paste0("nb_hesper_items_", list_group_name, ".", cat)) / !!rlang::sym("n_valid"),
      !!paste0("overall_prop_hesper_", list_group_name, ".", cat) := !!rlang::sym(paste0("nb_hesper_items_", list_group_name, ".", cat)) /  !!rlang::sym("all_serious_items"),
