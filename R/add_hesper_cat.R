...
# #'
# #' @description naming done using the following syntax: nb_hesper_items_section_name.section_category
# #'
...
# #'
# #' @return dataframe with following new columns added: nb_hesper_items_section_name.section_category // prop_hesper_items_section_nae // hesper_items_section_nae
...
#   df,
#   list_group = list(
#     aap = c("hesper_information", "hesper_aid"),
#     education = c("hesper_education"),
#     fsl = c("hesper_food"),
#     health = c(
#       "hesper_health",
...
#       "hesper_drug"
#     ),
#     snfi = c("hesper_shelter", "hesper_clothes_etc"),
#     wash = c(
...
#   col_not_matching <- col_hesper |> purrr::keep(\(x) !x %in% colnames(df))
...
#   if (
#     df |>
#       dplyr::select(dplyr::any_of(col_hesper)) |>
#       purrr::map(unique) |>
#       purrr::map(~ all(!. %in% c(choice_serious, choice_no_serious))) |>
#       unlist() |>
#       any()
#   ) {
#     stop(
#       "The specified values for arguments choice_serious and choice_no_serious do not appear in the dataframe. Please check the values."
#     )
#   }
...
#     df <- dplyr::mutate(
#       df,
#       "all_serious_items" := rowSums(
#         dplyr::across(
#           dplyr::any_of(unlist(unname(list_group))),
#           ~ . %in% choice_serious
#         ),
#         na.rm = T
#       ),
#       "n_valid" := rowSums(dplyr::across(
#         dplyr::any_of(list_group[[cat]]),
#         ~ !. %in% choice_exclude
#       )),
#       !!paste0(
#         "nb_hesper_items_",
...
#         ".",
#         cat
#       ) := !!rlang::sym(paste0("nb_hesper_items_", list_group_name, ".", cat)) /
#         !!rlang::sym("n_valid"),
#       !!paste0(
#         "overall_prop_hesper_",
#         list_group_name,
#         ".",
#         cat
#       ) := !!rlang::sym(paste0("nb_hesper_items_", list_group_name, ".", cat)) /
#         !!rlang::sym("all_serious_items"),
#       !!paste0(
#         "at_least_one_hesper_item_",
#         list_group_name,
#         ".",
#         cat
#       ) := dplyr::case_when(
...
#   df <- dplyr::select(df, -dplyr::all_of(c("n_valid")))
