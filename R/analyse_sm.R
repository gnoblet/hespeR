...
analyse_sm_ci <- function(df, group_var = NULL, var, col_weight) {
  if (any(!var %in% colnames(df))) {
    print(paste0(
      "Colnames ",
      paste0(var[!var %in% colnames(df)], collapse = "; "),
      " not in dataset. Will be excluded from analysis"
    ))
  ...
  df <- df |>
    ...
    dplyr::summarise(dplyr::across(
      dplyr::all_of(var),
      list(
        mean = ~ survey_mean(., vartype = "ci", na.rm = T),
        count = ~ sum(., na.rm = T),
        n = ~ sum(!is.na(.))
      )
    )) |>
    dplyr::rename_with(
      .fn = ~ stringr::str_replace_all(., c("_(?=(low|upp))" = "\\/"))
    ) |>
    tidyr::pivot_longer(where(is.numeric)) |>
    tidyr::separate(
      name,
      c("question", "choice.key"),
      sep = "\\.",
      remove = T
    ) |>
    tidyr::separate(
...
analyse_sm_quick <- function(df, group_var = NULL, var, col_weight = NULL) {
  ...
  df <- df |>
    ...
    dplyr::summarise(dplyr::across(
      dplyr::all_of(var),
      list(
        mean = ~ weighted.mean(., w = !!rlang::sym(col_weight), na.rm = T),
        count = ~ sum(., na.rm = T),
        n = ~ sum(!is.na(.))
      )
    )) |>
