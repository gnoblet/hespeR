...
clean_top_priorities_subset <- function(
  ...
  if (!all(purrr::map_lgl(subset_cols_vals, ~is_not_empty(.x)))) {
  ...
  if (!all(purrr::map_lgl(names(subset_cols_vals), ~all(unlist(map(subset_cols_vals[[.x]], ~.[["subset_val"]])) %in% unique(data[[.x]]))))){
  ...
  if (!all(unlist(purrr::map_depth(subset_cols_vals, 2, ~ .x$col_val)) %in% unique(unlist(data[, .SD, .SDcols = col_prio])))){
